...
Nearcade depende de um processo secundário Python separado (`input_driver.py`) para lidar com a injeção de entrada no nível do sistema operacional. O servidor Node.js recebe WebSockets contendo arrays binários de API do Gamepad, descompacta-os em estruturas JSON padrão e os canaliza para Python via `stdin`.
...
A captura de telas no Linux é notoriamente fragmentada. Nearcade aproveita o `desktopCapturer` do Electron juntamente com sinalizadores Chromium modernos para suportar compositores X11 e Wayland sem problemas.
...
Se o aplicativo Electron for fechado à força, o Node.js pode deixar túneis `cloudflared` órfãos ou portas TCP travadas. Na inicialização, o Nearcade usa `kill-port` para limpar a porta 3000, garantindo que o servidor Express possa se ligar de forma limpa.

...
Le routeur VPS Nearcade (répertoire `/vps`) gère la signalisation WebSocket et le proxy du trafic de négociation WebRTC.
Sur votre VPS, téléchargez la version Nearcade et exécutez le routeur :
```bash
./nearcade-router --port 8080
```

### 3. Connecter l'hôte
Dans les paramètres de l'application Nearcade, sous **Dedicated Tunnel Provider**, configurez votre IP et votre port VPS.

...
Der Nearcade VPS-Router (Verzeichnis `/vps`) verarbeitet die WebSocket-Signalisierung und den Proxy-WebRTC-Handshake-Verkehr.
Laden Sie auf Ihrem VPS die Nearcade-Version herunter und führen Sie den Router aus:
```bash
./nearcade-router --port 8080
```

### 3. Host verbinden
Konfigurieren Sie in den Einstellungen der Nearcade-App unter **Dedicated Tunnel Provider** Ihre VPS-IP und Ihren VPS-Port.

...
O roteador Nearcade VPS (diretório `/vps`) lida com sinalização WebSocket e proxy do tráfego de handshake WebRTC.
No seu VPS, baixe a versão Nearcade e execute o roteador:
```bash
./nearcade-router --port 8080
```

### 3. Conecte o host
Nas configurações do aplicativo Nearcade, em **Provedor de túnel dedicado**, configure seu IP e porta VPS.
